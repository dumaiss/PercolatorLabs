...
/* Function:  vrEmuTms9918Text2ScanLine
 * ----------------------------------------
 * generate a Text 2 mode scanline (80 columns, 480 pixels wide)
 */
static void __time_critical_func(vrEmuTms9918Text2ScanLine)(VrEmuTms9918* tms9918, uint8_t y, uint8_t pixels[TMS9918_PIXELS_X])
{
  const uint8_t tileY = y >> 3;   /* which name table row (0 - 23) */
  const uint8_t pattRow = y & 0x07;  /* which pattern row (0 - 7) */
...
VR_EMU_TMS9918_DLLEXPORT void __time_critical_func(vrEmuTms9918ScanLine)(VrEmuTms9918* tms9918, uint8_t y, uint8_t pixels[TMS9918_PIXELS_X])
{
  ...
  switch (tms9918->mode)
  {
    ...
    case TMS_MODE_TEXT_2:
      vrEmuTms9918Text2ScanLine(tms9918, y, pixels);
      break;
